...
class ExtractSuper(object):
    def __init__(self):
        self.fe = SuperPointFrontend(weights_path='./superpoint_v1.pth', nms_dist=2, conf_thresh=0.001, nn_thresh=0.7,
                                     cuda=True)
...
if __name__ == "__main__":
    ...
    if opt.suffix == 'sift-2000':
        ...
        listing = []
        for seq in ['buckingham_palace', 'notre_dame_front_facade', 'reichstag', 'sacre_coeur']:
            search = os.path.join(opt.input_path, f'{seq}/test/images/*.jpg')
            listing += glob.glob(search)
