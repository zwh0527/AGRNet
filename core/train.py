...
def train(model, train_loader, valid_loader, config):
    model.cuda()
    optimizer = optim.AdamW(model.parameters(), lr=config.train_lr, weight_decay=config.weight_decay)
    scheduler = optim.lr_scheduler.StepLR(optimizer, 200000, 0.5)
    ...
    if config.resume:
        ...
        logger_train.set_names(
            ['Learning Rate'] + ['Geo Loss', 'Classfi Loss', 'L2 Loss'] * (
                config.iter_nums + 1 if config.is_final_predictor else config.iter_nums))
    ...
    with trange(start_epoch, config.train_iter, ncols=config.tqdm_width) as _tqdm:
        for step in range(start_epoch, config.train_iter):
            ...
            if (step + 1) % config.log_intv == 0:
                _tqdm.update(config.log_intv)
